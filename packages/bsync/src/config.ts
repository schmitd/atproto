...
export const envToCfg = (env: ServerEnvironment): ServerConfig => {
  ...
  if (env.revenueCatV1ApiKey) {
    ...
    assert(
      ...
      'missing Stripe Product ID for yearly subscription',
    )
    assert(
    ...
    purchasesCfg = {
      ...
      stripePriceIdYearly: env.stripePriceIdYearly,
      stripeProductIdMonthly: env.stripeProductIdMonthly,
      stripeProductIdYearly: env.stripeProductIdYearly,
    }
  }
...
export const readEnv = (): ServerEnvironment => {
  return {
    ...
    stripePriceIdYearly: envStr('BSYNC_STRIPE_PRICE_ID_YEARLY'),
    stripeProductIdMonthly: envStr('BSYNC_STRIPE_PRODUCT_ID_MONTHLY'),
    stripeProductIdYearly: envStr('BSYNC_STRIPE_PRODUCT_ID_YEARLY'),
  }
}
